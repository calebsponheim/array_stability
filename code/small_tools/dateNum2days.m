...
elseif numel(num2str(date))==8
    date_temp = num2str(date);
    y = str2double(date_temp(3:4));
    m = str2double(date_temp(5:6));
    d = str2double(date_temp(7:8));
    days = 365*y + 30*m + d;
    
else
    fprintf('dateString2days.m: dateString %i length is not 6, or 5. Returning 0 days. Check your data. \n',date)
    days = 0;
end
end%function